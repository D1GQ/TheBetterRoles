...
internal class MersenneTwister : IRandom
{
    internal MersenneTwister() : this((Int32)DateTime.UtcNow.Ticks) { }
    internal MersenneTwister(Int32 seed)
    {
        Init((UInt32)seed);
    }
    ...
    public float Next(float minValue, float maxValue)
    {
        ...
        int randomInt = Next(int.MinValue, int.MaxValue);
        float normalized = (randomInt - (float)int.MinValue) / ((float)int.MaxValue - int.MinValue);

        return minValue + normalized * (maxValue - minValue);
    }

    public float Next(float maxValue) => Next(0f, maxValue);
}