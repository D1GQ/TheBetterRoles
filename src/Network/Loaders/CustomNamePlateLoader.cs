...
/// <summary>
/// The CustomNamePlateLoader class handles the downloading, configuration, and management of custom name plates for the game.
/// It fetches name plate configuration data and assets from a remote repository, manages the download and 
/// installation of the name plate assets, and ensures that they are available for use in the game.
...
/// It also integrates with the CustomHatManager to load the downloaded name plates.
/// </summary>
internal class CustomNamePlateLoader : MonoBehaviour, IGithubLoader
{
    public string RepositoryUrl => "https://raw.githubusercontent.com/D1GQ/TBR_Hats/main";
    private const string ManifestFileName = "manifest.json";
    private const string ManifestName = "Nameplates";
    private readonly string NamePlatesDirectory = TBRDataManager.NamePlatesFolder;

    /// <summary>
    /// Coroutine to fetch the manifest from the remote repository and check the availability of name plates.
    /// If the name plates are missing, it will download them and ensure that they are correctly stored for use in-game.
    /// </summary>
    [HideFromIl2Cpp]
    internal IEnumerator CoFetchNamePlates()
    {
        int count = 0;
        float hang = 0;
        ...
        NetworkLogger.Log($"Name Plates {ManifestFileName} loaded! Name Plates available: {response[ManifestName].Count}");

        if (!Directory.Exists(NamePlatesDirectory)) Directory.CreateDirectory(NamePlatesDirectory);

        List<string> toDownload = GenerateDownloadList(response[ManifestName]);

        NetworkLogger.Log($"Downloading {toDownload.Count} Name Plates.");

        foreach (var folderName in toDownload)
        {
            yield return CoDownloadNamePlateFolder(folderName);
        }

        CustomHatManager.RegisterNamePlates();
    ...
    /// <summary>
    /// Generates a list of name plates that need to be downloaded based on missing configuration or sprite assets.
    /// Ensures that any incomplete or missing name plate data is redownloaded to maintain consistency.
    /// </summary>
    [HideFromIl2Cpp]
    private List<string> GenerateDownloadList(IEnumerable<string> availableFolders)
    {
        ...
        {
            var folderPath = Path.Combine(NamePlatesDirectory, folderName);
            ...
            try
            {
                var config = JsonSerializer.Deserialize<CustomNamePlateData>(File.ReadAllText(configFilePath));
    ...
    /// <summary>
    /// Downloads a specific name plate folder, including its configuration and sprite assets.
    /// Ensures the directory structure is created and all necessary files are retrieved.
    /// </summary>
    [HideFromIl2Cpp]
    private IEnumerator CoDownloadNamePlateFolder(string folderName)
    {
        ...
        CustomNamePlateData config;
        try
        {
            config = JsonSerializer.Deserialize<CustomNamePlateData>(wwwConfig.downloadHandler.text);
        }
        catch (Exception ex)
        {
        ...
        List<string> filesToDownload = [config.Sprite];
