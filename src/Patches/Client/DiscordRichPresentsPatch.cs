...
internal class DiscordRichPresentsPatch
{
    ...
    [HarmonyPrefix]
    internal static void UpdateActivity_Prefix([HarmonyArgument(0)] Activity activity)
    {
        ...
        try
        {
            ...
            else
            {
                if (GameState.IsHideNSeek)
                    details = $"TBR v{ModInfo.PluginVersion} - Hide & Seek";
            }
        }
        catch
        {
