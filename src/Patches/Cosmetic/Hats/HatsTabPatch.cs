...
internal static class HatsTabPatch
{
    ...
    private static bool OnEnablePrefix(HatsTab __instance)
    {
        ...
        var unlockedHats = HatManager.Instance.GetUnlockedHats().Where(hat => hat.TryGetConfig().CheckPermission());
    ...
    private static Dictionary<string, List<Tuple<HatData, CustomHatData>>> CreateHatPackages(IEnumerable<HatData> hats)
    {
        ...
        foreach (var hat in hats)
        {
            ...
            var packageKey = ext?.Package ?? CustomHatManager.InnerslothHatPackageName;
    ...
    private static int PackagePriority(string package)
    {
        ...
        {
            CustomHatManager.InnerslothHatPackageName => 1000,
            CustomHatManager.DeveloperHatPackageName => 0,
            _ => 500
        };
    }
    ...
    {
        var offset = yStart;
        var isDefaultPackage = packageName == CustomHatManager.InnerslothHatPackageName;
