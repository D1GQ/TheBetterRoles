...
internal class TaskAdderGamePatch
{
    ...
    private static Dictionary<TaskAddButton, RoleClass> rolesForButtons = [];

    [HarmonyPatch(nameof(TaskAdderGame.ShowFolder))]
    [HarmonyPrefix]
    private static bool ShowFolder_Prefix(TaskAdderGame __instance, [HarmonyArgument(0)] TaskFolder taskFolder)
    {
        ...
        for (int m = 0; m < CustomRoleManager.RolePrefabs.Length; m++)
        {
            RoleClass roleBehaviour = CustomRoleManager.RolePrefabs[m];

            TaskAddButton taskAddButton = UnityEngine.Object.Instantiate(__instance.RoleButton);
